...
export interface CleanCiReceipt {
  ...
  readonly dependency_command: "pnpm install --frozen-lockfile --prod=false --ignore-scripts";
...
function initializeExactGitMetadata(
  source: string,
  destination: string,
  ...
  run(command, "git", ["--no-optional-locks", "-C", destination, "symbolic-ref", "HEAD", "refs/heads/clean-ci-candidate"], destination, environment);
  run(command, "git", [
    "--no-optional-locks", "-C", destination, "update-ref", "refs/heads/clean-ci-candidate",
    capture.repository_object.head_commit,
  ], destination, environment);
  run(command, "git", ["--no-optional-locks", "-C", destination, "read-tree", "--empty"], destination, environment);
 ...
 */
export function createCleanCandidateCapsule(
  sourceRoot: string,
  capsuleRoot: string,
  options: Pick<CleanCiOptions, "command"> = {},
): { readonly repository: string; readonly source_capture: RepositoryObjectSurfaceCapture } {
  ...
  const command = options.command ?? execFileSync;
  ...
  initializeExactGitMetadata(source, repository, sourceCapture, command, environment);
...
export function runCleanCi(sourceRoot: string, options: CleanCiOptions = {}): CleanCiReceipt {
  const source = resolve(sourceRoot);
  const command = options.command ?? execFileSync;
  const capsule = mkdtempSync(join(tmpdir(), "mister-clean-ci-capsule-"));
  let sourceBefore: RepositoryObject | undefined;
  let receipt: CleanCiReceipt | undefined;
  let primaryFailure: unknown;
  try {
    const materialized = createCleanCandidateCapsule(source, capsule, { command });
    ...
    run(command, "pnpm", ["install", "--frozen-lockfile", "--prod=false", "--ignore-scripts"], candidate, environment);
    ...
    run(command, "pnpm", ["run", "build:raw"], candidate, environment);
    ...
    for (const script of CLEAN_CI_MATRIX) run(command, "pnpm", ["run", script], candidate, environment);
    ...
    receipt = Object.freeze({
      ...
      dependency_command: "pnpm install --frozen-lockfile --prod=false --ignore-scripts",
    ...
    if (!options.keepCapsule) rmSync(capsule, { recursive: true, force: true });
    if (primaryFailure !== undefined && sourceFailure !== undefined) {
      throw new AggregateError([primaryFailure, sourceFailure], "clean CI and source-custody verification both failed");
    }
...
if (import.meta.main) {
  try {
    ...
    const receipt = runCleanCi(resolve(import.meta.dir, ".."));
