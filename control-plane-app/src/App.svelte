<script lang="ts">
  ...
  $: matchingAgents = snapshot && topologyMeasured ? snapshot.agents : [];
  ...
  $: debtFlowMeasured = snapshot ? hasCurrentMeasurement(snapshot, "current_debt_flow") : false;
  $: topologyMeasured = snapshot ? hasCurrentMeasurement(snapshot, "live_topology") : false;
  $: payableIssues = (snapshot?.issues ?? []).filter((issue) => issue.state !== "paid" && issue.state !== "false_positive");
  $: technicalDebt = debtFlowMeasured ? payableIssues.filter((issue) => issue.technical_or_agentic === "technical").length : null;
  $: agenticDebt = debtFlowMeasured ? payableIssues.filter((issue) => issue.technical_or_agentic === "agentic_operational").length : null;
  $: meanIssueConfidence = debtFlowMeasured && payableIssues.length > 0 ? payableIssues.reduce((sum, issue) => sum + issue.confidence, 0) / payableIssues.length : null;
  ...
  function profileValue(field: { readonly value: unknown; readonly provenance: { readonly status: string } } | null | undefined): string {
    if (field === undefined || field === null) return "NOT RECORDED";
    if (field.value === null) return "UNKNOWN";
    if (Array.isArray(field.value)) return field.value.length === 0 ? "none recorded" : field.value.join(", ");
    if (typeof field.value === "object") return JSON.stringify(field.value);
    return String(field.value);
  }

  function profileStatus(field: { readonly provenance: { readonly status: string } } | null | undefined): string {
    return field?.provenance.status ?? "unavailable";
  }
  ...
  function issueRouting(issue: SnapshotIssue) {
    return rankSnapshotAgents(matchingAgents, [{ capability_id: capabilityForIssue(issue), weight: 1 } as CapabilitySelection]);
  }
    ...
    <main class="main" bind:this={mainElement} tabindex="-1" aria-label={activeView}>
      ...
      {:else if snapshot}
        {#if activeView === "Progress / Command"}
          <p class="eyebrow">Repository state · {snapshot.current_subject.observed_at}</p><h2 class="view-heading">Are we moving closer?</h2><p class="view-lead">{debtFlowMeasured ? "The headline is calculated from one identity-bound debt flow. Detection expansion, classification, and remediation harm remain separate." : "Partial current observation: total debt, no-harm, topology, and complexity are NOT MEASURED until their own current evidence is bound."}</p>
          <section class="hero" aria-label="Current progress">
            <div class="hero-copy">
              <p class="eyebrow">Outstanding real issues</p>
              <p class="hero-number">{debtFlowMeasured ? snapshot.current_flow.ending_real_issues : "NOT MEASURED"}<small>{terminal?.verdict === "CLEAN" ? "CLEAN" : `NOT CLEAN · ${terminal?.evidence_state ?? "UNKNOWN"}`} · terminal contract projection</small></p>
              <span class:no-harm-fail={currentNoHarm(snapshot) === false} class:status-pass={currentNoHarm(snapshot) === true} class="status-info">No-harm: {currentNoHarm(snapshot) === null ? "NOT ESTABLISHED" : currentNoHarm(snapshot) ? "holding" : "failed"}</span>
              <p class="view-lead">{debtFlowMeasured ? `${snapshot.current_flow.ending_real_issues} payable issues remain. ${snapshot.current_flow.caused_by_remediation} caused by remediation. ${snapshot.current_flow.boundary_blocked} boundary-blocked.` : "The retained issue graph is not a current total; no payable, caused, or boundary count is inferred."}</p>
            ...
            <div class="hero-figure">
              <p class="eyebrow">{debtFlowMeasured ? `Debt flow · ${snapshot.current_run_id}` : "Partial current observation"}</p>
              {#if debtFlowMeasured}
              ...
              {:else}<p class="muted">Debt-flow magnitudes are withheld because no current census is bound.</p>{/if}
              <p class="muted">{currentNoHarm(snapshot) === null ? "No-harm is NOT ESTABLISHED: an observation receipt is not a repository-object census." : `No-harm is ${currentNoHarm(snapshot) ? "holding" : "failed"}; the same object digest binds this reading.`}</p>
            </div>
          </section>
          <div class="metric-grid"><div class="metric"><strong>{snapshot.first_flow?.ending_real_issues ?? "NOT MEASURED"}</strong><span>First run</span></div><div class="metric"><strong>{snapshot.previous_flow?.ending_real_issues ?? "NOT MEASURED"}</strong><span>Previous run</span></div><div class="metric"><strong>{debtFlowMeasured ? snapshot.current_flow.ending_real_issues : "NOT MEASURED"}</strong><span>Current run</span></div><div class="metric"><strong>{debtFlowMeasured ? snapshot.current_flow.paid : "NOT MEASURED"}</strong><span>Paid this run</span></div></div>
          <section class="section split" aria-label="Debt composition and operating posture">
            ...
            <div class="panel"><h3>Team, topology, and authority</h3><ul class="list"><li><span>Active repository agents</span><strong>{topologyMeasured ? activeAgents.length : "NOT OBSERVED"}</strong></li><li><span>Configured control surfaces</span><strong>{topologyMeasured ? activeControlSurfaces : "NOT OBSERVED"}</strong></li><li><span>Declared issue worktrees</span><strong>{topologyMeasured ? currentWorktrees : "NOT OBSERVED"}</strong></li><li><span>Unbound active identities</span><strong>{topologyMeasured ? unboundActiveAgents : "NOT OBSERVED"}</strong></li><li><span>Authority / mode</span><strong>{snapshot.current_authority} · {snapshot.manifest?.directive_state ?? "unbound preview"}</strong></li><li><span>Branch / target</span><strong>{snapshot.current_subject.branch} · {snapshot.manifest?.target_ref ?? "no manifest target"}</strong></li></ul></div>
          </section>
          <section class="section"><h3 class="section-title">Recommended next wave</h3><p class="section-caption">{debtFlowMeasured ? "Deterministic dependency planning over the current issue graph. Select issues in the workbench to build a dependency-closed ADVISE projection." : "Inherited partial-scope projection only: paid prerequisites are satisfied and never re-recommended; severity, confidence, and order are not fresh priority judgments."}</p><div class="split"><div class="panel"><ol class="list">{#each plannedIssues.slice(0, 3) as issue}<li><div><span class="eyebrow">{issue.issue_id} · {issue.plan.concurrency}</span><strong>{issue.title}</strong><p class="muted">{issue.recommended_tuple}</p></div><span class="status-info">{issue.plan.order || "blocked"}</span></li>{/each}</ol></div><div class="panel"><h3>Directive boundary</h3><p class="muted">{snapshot.manifest ? "Manifest-bound projection." : "Unbound ADVISE preview. Queue and delivery claims are prohibited."}</p><button class="control active" onclick={copyDirective}>Copy {snapshot.manifest ? "manifest projection" : "ADVISE preview"}</button>{#if copyMessage}<p class="notice" role="status">{copyMessage}</p>{/if}</div></div></section>
          <section class="section"><h3 class="section-title">Complexity &amp; shipping balance</h3><p class="section-caption">Repository-object, tracked, authored, generated, documentation, dependency, and ignored-live surfaces remain distinct. Thresholds are policy choices, not industry truth.</p><div class="metric-grid"><div class="metric"><strong>{measuredComplexity(snapshot.complexity.tracked_object.bytes)}</strong><span>Tracked object bytes</span></div><div class="metric"><strong>{measuredComplexity(snapshot.complexity.authored_source.lines)}</strong><span>Authored source lines</span></div><div class="metric"><strong>{complexityRatio(snapshot.complexity.docs_to_authored_code.lines)}</strong><span>Docs:code lines</span></div><div class="metric"><strong>{complexityFunction(snapshot.complexity.functions.p95)}</strong><span>Function p95</span></div></div>
            ...
            <details class="panel disclosure"><summary>Object binding, coverage, limits, and longitudinal trend</summary><p class="muted">Availability: {availabilityLabel(snapshot.complexity.availability)} · object: {snapshot.complexity.object_sha256 ?? "UNKNOWN"}</p><p>{snapshot.complexity.structural_coverage}</p><p class="muted">{snapshot.complexity.limitations.join(" ")}</p><p class="tuple">Cycles: {snapshot.complexity.cycles.join("; ") || "none measured"}. Hotspots: {snapshot.complexity.hotspots.join("; ") || "none measured"}.</p>{#if snapshot.complexity.trend.length === 0}<p class="muted">Longitudinal complexity is NOT MEASURED.</p>{:else}<p class="muted">{hasCurrentMeasurement(snapshot, "current_complexity") ? "Longitudinal complexity" : "Historical complexity only — not projected onto the current observation"}.</p><ul class="list">{#each snapshot.complexity.trend as point}<li><span>{point.run_id} · {point.object_sha256}</span><strong>{point.complexity ?? "NOT MEASURED"} complexity · {point.docs_to_code_lines === null ? "NOT MEASURED" : `${Math.round(point.docs_to_code_lines * 100)}%`} docs:code</strong></li>{/each}</ul>{/if}</details></section>
        ...
        {:else if activeView === "Agent Finder"}
          ...
          <div class="agent-grid">{#each ranked.ranked as item}<article class="agent-card"><span class="status-pass">Ranked · {item.weighted_score.toFixed(3)}</span><h3>{item.agent.model}</h3><p class="tuple">{formatAgentTuple(item.agent)}</p><p class="muted">{item.agent.inference_source} · {item.agent.route}</p><p><strong>Primary qualification:</strong> {qualificationLabel(primaryQualification(item.agent, selections.map((selection) => selection.capability_id)))}</p><p>{humanState(item.agent.availability)} · {item.agent.active_in_repository ? "active in repository" : "global candidate"} · {item.agent.familiarity_runs} familiarity runs</p>{#each item.category_scores as score}{@const evidenceScore = selectedScore(item.agent, score.capability_id)}<div class="capability-evidence"><div class="capability-line"><span>{ALL_CAPABILITY_DEFINITIONS.find((definition) => definition.id === score.capability_id)?.label ?? score.capability_id}</span><strong>{score.weighted_score.toFixed(3)}</strong></div><p class="muted">Score {evidenceScore?.score.toFixed(3) ?? "N/A"} · confidence {evidenceScore ? percent(evidenceScore.confidence) : "N/A"} · weight {capabilityWeights[score.capability_id] ?? 0} · {evidenceScore?.verified_trials ?? 0} credited trials · {qualificationLabel(evidenceScore?.qualification ?? "UNTESTED")}</p>{#if evidenceScore}<p class="muted">{topologyMeasured ? `Verified ${evidenceScore.qualification_provenance.verified_successes}/${evidenceScore.verified_trials} · ${evidenceScore.qualification_provenance.repository_cohort_count} repositories · independent ${evidenceScore.qualification_provenance.independent_evaluation ? "yes" : "no"} · unresolved no-harm ${evidenceScore.qualification_provenance.unresolved_no_harm_violations} · authority ${evidenceScore.qualification_provenance.unresolved_authority_violations}` : "Historical qualification ledger retained; current no-harm and authority are NOT RE-EVALUATED."}</p>{/if}</div>{/each}<details class="disclosure agent-card-detail"><summary>Agent card · execution profile</summary><p class="muted"><strong>Harness:</strong> {item.agent.harness}; version {profileValue(item.agent.execution_profile?.harness.version)}. <strong>Model:</strong> {item.agent.model}; version {profileValue(item.agent.execution_profile?.model.version)} · release {profileValue(item.agent.execution_profile?.model.release_date)}. <strong>Reasoning:</strong> {item.agent.reasoning_level}; {item.agent.execution_profile ? `${item.agent.execution_profile.settings.length} settings recorded` : "settings NOT RECORDED"}.</p><p class="muted"><strong>Inference:</strong> {item.agent.inference_source} · deployment {profileValue(item.agent.execution_profile?.inference.deployment_mode)} · provider {profileValue(item.agent.execution_profile?.inference.provider)} · gateway {profileValue(item.agent.execution_profile?.inference.gateway)} · server {profileValue(item.agent.execution_profile?.inference.server)}. <strong>Invocation:</strong> {item.agent.invocation_adapter} · headless {item.agent.headless === null ? "UNKNOWN" : item.agent.headless ? "PROVEN" : "NO"}.</p><p class="muted"><strong>Capability environment:</strong> tools {profileValue(item.agent.execution_profile?.capability_environment.tools)} · plugins {profileValue(item.agent.execution_profile?.capability_environment.plugins)} · MCP {profileValue(item.agent.execution_profile?.capability_environment.mcp_servers)} · skills {profileValue(item.agent.execution_profile?.capability_environment.skills)}. <strong>Context:</strong> start {profileValue(item.agent.execution_profile?.context.starting_context_tokens)} tokens · window {profileValue(item.agent.execution_profile?.context.context_window_tokens)}.</p><p class="muted"><strong>Current identity:</strong> {humanState(item.agent.execution_identity.disposition)} · intended surface {item.agent.execution_identity.intended_surface_label} · externally verified {item.agent.execution_identity.last_external_verification_at ?? "NOT VERIFIED"}. Evidence: {item.agent.execution_identity.evidence.map((ref) => ref.path).join(", ") || "INSUFFICIENT EVIDENCE"}.</p><p class="muted"><strong>Performance:</strong> cost/success {numberMetric(item.agent.metrics.cost_per_success_usd, " USD")} · tokens/success {numberMetric(item.agent.metrics.tokens_per_success)} · throughput {numberMetric(item.agent.metrics.tokens_per_second, " tok/s")} · reliability {percent(item.agent.metrics.reliability)} · TTFT {profileValue(item.agent.execution_profile?.performance.time_to_first_token_ms)}.</p></details></article>{/each}</div>{#if ranked.excluded.length > 0}<details class="panel disclosure"><summary>Excluded candidates · {ranked.excluded.length}</summary><ul class="list">{#each ranked.excluded as item}<li><span>{formatAgentTuple(item.agent)}</span><strong>{item.reason}</strong></li>{/each}</ul></details>{/if}
        {:else if activeView === "Team Roster"}
          <p class="eyebrow">{topologyMeasured ? "Repository and machine-global roster · grouped by family + harness" : "Historical roster inventory · grouped by family + harness"}</p><h2 class="view-heading">Seats, readiness, and evidence</h2><p class="view-lead">{topologyMeasured ? "Active repository groups appear first. Expand a group to inspect every exact model + harness + reasoning treatment." : "Historical records are retained, but current availability, control surfaces, and execution identity are NOT REVERIFIED."} Untested is not failed, and a historical qualification is not current route proof.</p>
          ...
          <div class="roster-groups">{#each rosterGroups as group}<details class="panel roster-group" open={topologyMeasured && group.active_count > 0}><summary><span><strong>{group.family} · {group.harness}</strong><small>{group.treatments.length} exact treatments</small></span><span>{topologyMeasured ? `${group.active_count} active · ${group.available_count} available` : "NOT OBSERVED · historical inventory"}</span></summary><div class="roster-treatment-grid">{#each group.treatments as agent}<article class="roster-treatment"><div><strong>{agent.model}</strong><p class="tuple">{agent.reasoning_level} · {agent.agent_tuple_id}</p></div><dl><div><dt>Role / surface</dt><dd>{agent.role}<br /><span class="muted">{agent.control_surface ?? "NOT CONFIGURED"}</span></dd></div><div><dt>Qualification</dt><dd>{qualificationLabel(primaryQualification(agent, []))}<br /><span class="muted">{Math.max(...agent.capability_scores.map((score) => score.verified_trials), 0)} max credited trials</span></dd></div><div><dt>Availability</dt><dd>{humanState(agent.availability)}<br /><span class="muted">{topologyMeasured ? (agent.active_in_repository ? "active in repository" : "machine-global candidate") : "historical status; NOT REVERIFIED"}</span></dd></div><div><dt>Route / inference</dt><dd>{agent.route}<br /><span class="muted">{agent.inference_source}</span></dd></div></dl><details class="disclosure agent-card-detail"><summary>Agent card · {humanState(agent.execution_identity.disposition)}</summary><p class="muted">Harness {agent.harness}; version {profileValue(agent.execution_profile?.harness.version)}. Model {agent.model}; version {profileValue(agent.execution_profile?.model.version)} · release {profileValue(agent.execution_profile?.model.release_date)}. Reasoning {agent.reasoning_level}; {agent.execution_profile ? `${agent.execution_profile.settings.length} settings recorded` : "settings NOT RECORDED"}.</p><p class="muted">Deployment {profileValue(agent.execution_profile?.inference.deployment_mode)}; inference {profileValue(agent.execution_profile?.inference.provider)}; adapter {agent.invocation_adapter}; gateway {profileValue(agent.execution_profile?.inference.gateway)} · server {profileValue(agent.execution_profile?.inference.server)}. Intended surface {agent.execution_identity.intended_surface_label}; externally verified {agent.execution_identity.last_external_verification_at ?? "NOT VERIFIED"}. Evidence: {agent.execution_identity.evidence.map((ref) => ref.path).join(", ") || "INSUFFICIENT EVIDENCE"}.</p></details></article>{/each}</div></details>{/each}</div>
        ...
        {:else if activeView === "Raw Agent Inventory"}
          <p class="eyebrow">{topologyMeasured ? "Machine-global inventory · exact tuples" : "Historical inventory · current availability and identity NOT REVERIFIED"}</p><h2 class="view-heading">All observed agent tuples</h2><p class="view-lead">Every model + harness + reasoning level + execution route remains a separate treatment. Self-reported labels do not establish execution identity. Unavailable measures remain NOT MEASURED rather than being estimated.</p>
            ...
            {#each sortedRuns as run}
              ...
              <article class="run-card">
                <header class="run-card-header">
                  ...
                  <div class="run-verdict"><span class:status-pass={run.terminal_verdict === "CLEAN"} class:status-risk={run.terminal_verdict !== "CLEAN"}>{humanState(run.start_verdict)} → {humanState(run.terminal_verdict)}</span><strong>{run.run_id === snapshot.current_run_id && !debtFlowMeasured ? "NOT MEASURED" : run.debt_flow.ending_real_issues}</strong><small>{run.run_id === snapshot.current_run_id && !debtFlowMeasured ? "partial observation" : "remaining"}</small></div>
                </header>
                {#if run.run_id !== snapshot.current_run_id || debtFlowMeasured}<div class="run-flow" aria-label={`Debt flow for ${run.run_id}`}>
                ...
                </div>{:else}<p class="muted">Current debt-flow magnitudes are NOT MEASURED in this partial observation.</p>{/if}
                <p>{run.note}</p>
                <details class="disclosure run-detail">
                  ...
                  <div class="run-detail-grid">
                    ...
                    <section><h4>{run.run_id === snapshot.current_run_id ? currentBindingLabel(snapshot) : "Repository object"}</h4><p>{run.subject.branch} · {run.subject.commit} · {run.subject.tree}</p><p class="tuple">{run.run_id === snapshot.current_run_id ? snapshot.current_observation.evidence_binding.sha256 : run.subject.repository_object_sha256 ?? "NOT MEASURED"}</p><p><strong>Scope:</strong> {run.scope.join(", ") || "NOT RECORDED"}. <strong>Excluded:</strong> {run.exclusions.join(", ") || "none"}.</p></section>
                  ...
                  <p class="muted">{run.run_id === snapshot.current_run_id ? topologyMeasured ? `Current topology: ${activeAgents.length} active agents · ${activeControlSurfaces} control surfaces · ${currentWorktrees} declared issue worktrees · ${unboundActiveAgents} unbound identities.` : "Current topology is NOT OBSERVED in this partial observation; historical roster records are retained without a live availability or identity claim." : `Historical topology is NOT RECORDED in snapshot ${CONTROL_PLANE_SNAPSHOT_SCHEMA_VERSION}; current team is not projected backward.`}</p>
                </details>
              </article>
            {/each}
          </div>
        {:else}
          ...
          <div class="split"><section class="panel"><h3>Read path and versions</h3><ul class="list"><li><span>Source</span><strong>{snapshot.source_label}</strong></li><li><span>{currentBindingLabel(snapshot)}</span><strong>{snapshot.current_observation.evidence_binding.sha256}</strong></li><li><span>Mister Clean / detector</span><strong>{currentRun?.mister_clean_version ?? "UNKNOWN"} · {currentRun?.detector_version ?? "UNKNOWN"}</strong></li><li><span>Snapshot / taxonomy</span><strong>{CONTROL_PLANE_SNAPSHOT_SCHEMA_VERSION} · {snapshot.capabilities[0]?.taxonomy_version ?? "UNKNOWN"}</strong></li><li><span>Issue graph</span><strong>{snapshot.manifest ? `${snapshot.manifest.issue_graph.issue_graph_id} v${snapshot.manifest.issue_graph.version}` : "NOT AVAILABLE — no manifest"}</strong></li><li><span>Runtime identity</span><strong>NOT AVAILABLE in snapshot schema {CONTROL_PLANE_SNAPSHOT_SCHEMA_VERSION}</strong></li><li><span>Detector coverage</span><strong>{snapshot.detector_coverage}</strong></li><li><span>Evidence freshness</span><strong>{snapshot.evidence_freshness}</strong></li></ul></section><section class="panel"><h3>Authority and unavailable evidence</h3><ul class="list"><li><span>Authority</span><strong>{snapshot.current_authority}</strong></li><li><span>Directive</span><strong>{snapshot.manifest ? `${snapshot.manifest.directive_state} · ${snapshot.manifest.manifest_id}` : "unbound ADVISE preview"}</strong></li><li><span>Integration</span><strong>{snapshot.manifest?.receipt_boundary ?? "manifest + lease + target CAS required"}</strong></li><li><span>Historical topology</span><strong>{topologyMeasured ? "NOT RECORDED; never inferred from current agents" : "Historical inventory retained; current availability and identity NOT REVERIFIED"}</strong></li><li><span>Performance metrics</span><strong>NOT MEASURED until ledger-derived projection exists</strong></li><li><span>Secrets</span><strong>References only; never in snapshot/UI</strong></li></ul></section></div>
